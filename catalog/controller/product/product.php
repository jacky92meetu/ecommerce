...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {                                
			...
			$this->data['reward'] = floor($product_info['special']/10)*100;
			...
			$this->load->model('tool/image');
                                       ...
                                       $this->data['zoom_image'] = $this->model_tool_image->resize($product_info['image']);
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
                                                                'zoom_image' => $this->model_tool_image->resize($result['image']),
			...
			$results = $this->model_catalog_product->getProductRelated($this->request->get['product_id']);
                                       ...
                                       if(isset($category)){
                                           $data = array(
                                                    'filter_category_id' => $category[0]['category_id'],
                                                    'limit'              => 12
                                            );
				...
				foreach ($tags as $tag) {
					$this->data['tags'][] = array(
						...
						'href' => $this->url->link('product/search', 'tag=' . trim($tag))
                                       ...
                                       $merchant = $this->model_merchant_merchant->getStoreByProductId($this->request->get['product_id']);
                                       if($merchant){                                           
                                            $this->model_merchant_merchant->setCustomerOnline($this->customer->getId(),$merchant['store_id'],$this->request->get['product_id']);
                                           $this->data['merchant_detail'] = array(
                                                        'merchant_id' => $merchant['merchant_id'],
                                                        'store_id'      => $merchant['store_id'],
                                                        'thumb'          => $this->model_tool_image->resize($merchant['store_img_url'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height')),
                                                        'address'       => html_entity_decode($merchant['comp_address'], ENT_QUOTES, 'UTF-8'),
                                                        'contact_no'  => $merchant['comp_tel'],
                                                        'website'       => $merchant['comp_website'],
                                                        'name'              => $merchant['store_name'],
                                                        'merchant_url' => "mp/store?store_id=".$merchant['store_id'],
                                                        'package' => $merchant['package_id']
                                           );
                                           
                                           if($merchant['package_id']==0){
                                               $this->data['stock'] = "Out Of Stock";
                                               $this->data['merchant_detail'] = array_merge($this->data['merchant_detail'],array(
                                                   'address'       => "Non verified!",
                                                    'contact_no'  => "Non verified!",
                                                    'website'       => "Non verified!"
                                               ));
                                           }
                                           $merchant_products = $this->model_merchant_merchant->getAllProductByStoreId(array("store_id"=>$merchant['store_id'],"limit"=>6));                                           
                                           $this->data['merchant_products'] = array();
                                           foreach($merchant_products as $result){                                               
                                               if($result['product_id']==$this->request->get['product_id']){continue;}
                                               $len = 35;
                                                $product_name = html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8');                        
                                                if(utf8_strlen($product_name)>$len){                            
