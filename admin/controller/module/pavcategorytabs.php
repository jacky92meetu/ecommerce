...
class ControllerModulePavcategorytabs extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			unset($this->request->post['action']);
 			...
 			$this->model_setting_setting->editSetting('pavcategorytabs', $this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
			if ($mode == "save-stay") {
				$this->redirect($this->url->link('module/pavcategorytabs', 'token=' . $this->session->data['token'], 'SSL'));
			} else { 
		...
		$this->data['limit_tabs'] = $this->language->get('label_limit_tabs');
		...
		$this->data['status_nav'] = $this->language->get('label_status_nav');
		...
		$this->data['action'] = $this->url->link('module/pavcategorytabs', 'token=' . $this->session->data['token'], 'SSL');
		...
		$modules = array();
		if (isset($this->request->post['pavcategorytabs_module'])) {
			$modules = $this->request->post['pavcategorytabs_module'];
		} elseif ($this->config->get('pavcategorytabs_module')) { 
			$modules = $this->config->get('pavcategorytabs_module');
		}
		$this->data['modules'] = $modules;
		//$products = explode(',', $modules[$module_row]['related_product']);
		
		
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavcategorytabs.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		...
		if (isset($this->request->post['pavcategorytabs_module'])) {
			foreach ($this->request->post['pavcategorytabs_module'] as $key => $value) {
