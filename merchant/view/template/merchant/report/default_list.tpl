                                        ...
                                        <?
                                            foreach($data_header as $key => $value){
                                                ...
                                                if(!isset($value['no_filter'])){
                                                    $selected = "";
                                                    if(isset($data_list['filter']['order']) && array_key_exists($fieldname, $data_list['filter']['order'])!==FALSE){
                                                        $selected = 'selected="selected"';
                                                    }                                            
                                        ?>
                                        <option value="<? echo $fieldname; ?>" <? echo $selected; ?>><? echo $value['display_name']; ?></option>
                                        <?      
                                                }
                                            }
                                        ?>                                    
                            ...
                            <div>
                                <?  if(isset($button_group)){
                                    foreach($button_group as $key => $value){ 
                                        ...
                                        $url_class = (isset($value['ajax_url']))?'ajax_url':'';
                                        $url_target = (isset($value['url_target']))?'target="'.$value['url_target'].'"':'';
                                        ...
                                        $url = '<a href="'.$value['url'].'" '.$url_target.' class="btn btn-white '.$class.' '.$confirm_class.' '.$url_class.'" '.$confirm_text.' data-rel="tooltip"  title="" data-original-title="'.$value['display_name'].'">'.$icon.' '.$value['display_name'].'</a>';
                                        echo $url;
                                    }
                                    }
                                ?>                               
                    ...
                    <table class="table table-striped table-bordered table-hover dataTable">
                        <thead>
                            <tr>                            
                                <?
                                    foreach($data_header as $key => $value){
                                        ...
                                        if(!isset($value['no_filter'])){
                                            $sort_class = "sorting";                                    
                                            $sort_order = "asc";                                    
                                            if(array_key_exists($fieldname, $data_list['filter']['order'])){
                                                if($data_list['filter']['order'][$fieldname]=="desc"){
                                ...
                                ?>
                                <th class="<? echo $sort_class; ?> <? echo $class; ?>" sort_name="<? echo $fieldname; ?>" sort_order="<? echo $sort_order; ?>" <? echo $width; ?>>
                                    <? echo $icon; ?>
                            ...
                            <tr class="table_search_form">
                                <?
                                    ...
                                    if(!isset($value['no_search'])){
                                        $selected = "";
                                        $class = "";
                                        if(array_key_exists($fieldname,$data_list['filter']['where'])!==FALSE){
                                            $selected = $data_list['filter']['where'][$fieldname];
                                        }
                                        if(isset($value['status_value'])){
                                            $search_form = '<SELECT name="filter_search['.$fieldname.']">';
                                            ...
                                            $search_form = '<input type="text" name="filter_search['.$fieldname.']" value="'.$selected.'">';
                                        }                                        
                                    }
                                ?>
                                    ...
                                    if(isset($record[$key])){                                        
                                        ...
                                        if(isset($value['url'])){
											...
											$confirm_text = (isset($value['confirm']))?'confirm_text="'.$value['confirm'].'"':'';
                                                                                        $url_class = (isset($value['ajax_url']))?'ajax_url':'';
                                                                                        $url_target = (isset($value['url_target']))?'target="'.$value['url_target'].'"':'';
                                            $data_value = '<a href="'.$value['url'].'" '.$url_target.' class="'.$confirm_class.' '.$url_class.'" '.$confirm_text.'>'.$data_value.'</a>';
                                        }
                                            ...
                                            foreach($tool_group as $key => $value){ 
                                                ...
                                                if(isset($value['url'])){
													...
													$confirm_text = (isset($value['confirm']))?'confirm_text="'.$value['confirm'].'"':'';
                                                                                                        $url_class = (isset($value['ajax_url']))?'ajax_url':'';
                                                                                                        $url_target = (isset($value['url_target']))?'target="'.$value['url_target'].'"':'';
													$url = '<a href="'.$value['url'].'" '.$url_target.' class="tooltip-info '.$class.' '.$confirm_class.' '.$url_class.'" '.$confirm_text.' data-rel="tooltip"  title="" data-original-title="'.$value['display_name'].'">'.$icon.'</a>';
...
<script type="text/javascript">
    jQuery(function($){
        $('a.ajax_url').click(function(e){
            e.preventDefault();
            var obj = $(this);
            widget_toggle(obj,"show");
            $.ajax({
                type: 'GET',
                url: $(this).attr('href'),
                complete: function(xhr,status){
                    widget_toggle(obj,"hide");
                },
                success: function(data,status,xhr){
                    ...
                    if(typeof data.status === 'string' && data.status=='success' && typeof data.content==='string'){
                        if(typeof bootbox === 'object'){                                                        
